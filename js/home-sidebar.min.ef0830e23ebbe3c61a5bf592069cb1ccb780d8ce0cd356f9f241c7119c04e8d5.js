(function(){"use strict";function t(){const e=document.getElementById("clock-time"),t=document.getElementById("clock-date"),n=document.getElementById("clock-weekday");if(!e||!t||!n)return;const o=["日","一","二","三","四","五","六"];function s(){const s=new Date,i=String(s.getHours()).padStart(2,"0"),a=String(s.getMinutes()).padStart(2,"0"),r=String(s.getSeconds()).padStart(2,"0");e.textContent=`${i}:${a}:${r}`;const c=s.getFullYear(),l=String(s.getMonth()+1).padStart(2,"0"),d=String(s.getDate()).padStart(2,"0");t.textContent=`${c}年${l}月${d}日`;const u=o[s.getDay()];n.textContent=`星期${u}`}s(),setInterval(s,1e3)}function n(){const e=document.getElementById("weather-container");if(!e)return;const t={city:"Beijing",apiKey:"",apiUrl:"https://api.openweathermap.org/data/2.5/weather"};if(!t.apiKey){e.innerHTML=`
            ...
            `;return}async function n(){try{e.innerHTML='<div class="weather-loading">加载中...</div>';const o=`${t.apiUrl}?q=${t.city}&appid=${t.apiKey}&units=metric&lang=zh_cn`,n=await fetch(o);if(!n.ok)throw new Error("天气数据获取失败");const i=await n.json();s(i)}catch(t){console.error("天气获取错误:",t),e.innerHTML=`
                    <div class="weather-content">
                ...
                `}}function s(t){const n=o(t.weather[0].main),s=Math.round(t.main.temp),i=t.weather[0].description,a=t.main.humidity,r=t.wind?.speed||0;e.innerHTML=`
                <div class="weather-content">
                    <div class="weather-main">
                        <div class="weather-temp">${s}°C</div>
                        <div class="weather-icon">${n}</div>
                    </div>
                    <div class="weather-desc">${i}</div>
                    <div class="weather-details">
                        <div class="weather-detail-item">
                            <span>湿度：</span>
                            <span>${a}%</span>
