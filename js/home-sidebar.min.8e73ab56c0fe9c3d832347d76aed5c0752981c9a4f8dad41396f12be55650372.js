(function(){"use strict";function t(){const e=document.getElementById("clock-time"),t=document.getElementById("clock-date"),n=document.getElementById("clock-weekday");if(!e||!t||!n)return;const o=["日","一","二","三","四","五","六"];function s(){const s=new Date,i=String(s.getHours()).padStart(2,"0"),a=String(s.getMinutes()).padStart(2,"0"),r=String(s.getSeconds()).padStart(2,"0");e.textContent=`${i}:${a}:${r}`;const c=s.getFullYear(),l=String(s.getMonth()+1).padStart(2,"0"),d=String(s.getDate()).padStart(2,"0");t.textContent=`${c}年${l}月${d}日`;const u=o[s.getDay()];n.textContent=`星期${u}`}s(),setInterval(s,1e3)}function n(){const e=document.getElementById("weather-container"),n=document.querySelector(".home-sidebar");if(!e)return;const t={city:n?.dataset.weatherCity||"Beijing",apiKey:n?.dataset.weatherApiKey||"",apiUrl:n?.dataset.weatherApiUrl||"https://api.openweathermap.org/data/2.5/weather"};if(!t.apiKey){e.innerHTML=`
            ...
            `}function i(e){const t={Clear:"☀️",Clouds:"☁️",Rain:"🌧️",Drizzle:"🌦️",Thunderstorm:"⛈️",Snow:"❄️",Mist:"🌫️",Fog:"🌫️"};return t[e]||"🌤️"}s(),setInterval(s,30*60*1e3)}function s(){const d=document.querySelector(".music-widget");let t=[];if(d&&d.dataset.musicList)try{t=JSON.parse(d.dataset.musicList)}catch(e){console.error("音乐列表解析失败:",e)}(!t||t.length===0)&&(t=[]);const e=new Audio;let o=0,n=!1;const i=document.getElementById("music-title"),a=document.getElementById("music-artist"),r=document.getElementById("music-play-pause"),f=document.getElementById("music-prev"),p=document.getElementById("music-next"),c=document.getElementById("music-progress-bar"),b=document.getElementById("music-progress-fill"),g=document.getElementById("music-current-time"),s=document.getElementById("music-duration"),l=document.getElementById("music-volume"),m=document.getElementById("music-volume-value");if(!e||!r)return;function h(n){if(n<0||n>=t.length)return;o=n;const s=t[n];e.src=s.url,i&&(i.textContent=s.title),a&&(a.textContent=s.artist),e.load()}function j(){n?(e.pause(),r.textContent="▶",n=!1):(e.play().catch(e=>{console.error("播放失败:",e),alert("音乐播放失败，请检查音乐 URL 是否正确")}),r.textContent="⏸",n=!0)}function y(){const s=o-1<0?t.length-1:o-1;h(s),n&&e.play()}function v(){const s=(o+1)%t.length;h(s),n&&e.play()}function _(){if(e.duration){const t=e.currentTime/e.duration*100;b&&(b.style.width=t+"%"),g&&(g.textContent=u(e.currentTime)),s&&(s.textContent=u(e.duration))}}function u(e){if(isNaN(e))return"0:00";const t=Math.floor(e/60),n=Math.floor(e%60);return`${t}:${String(n).padStart(2,"0")}`}r.addEventListener("click",j),f&&f.addEventListener("click",y),p&&p.addEventListener("click",v),c&&c.addEventListener("click",t=>{const n=c.getBoundingClientRect(),s=(t.clientX-n.left)/n.width;e.currentTime=s*e.duration}),l&&(l.addEventListener("input",t=>{const n=t.target.value/100;e.volume=n,m&&(m.textContent=t.target.value+"%")}),e.volume=l.value/100),e.addEventListener("timeupdate",_),e.addEventListener("ended",v),e.addEventListener("loadedmetadata",()=>{s&&(s.textContent=u(e.duration))}),t.length>0?h(0):(i&&(i.textContent="暂无音乐"),a&&(a.textContent="请在配置中添加音乐"))}function e(){document.querySelector(".page.home")&&(t(),n(),s())}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",e):e()})()